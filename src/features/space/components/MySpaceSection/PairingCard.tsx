 ...
 * e.g., ['2026-02', '2026-03', '2026-04'] -> "Feb – Apr"
 */
...
export function PairingCard({
  ...
  const handleCardClick = (e: MouseEvent<HTMLDivElement>) => {
    // Only trigger if clicking the card itself, not interactive elements
  ...
  return (
    <motion.div
      ...
      aria-label={onCardClick ? `View details for ${user.name} - click to extend relationship` : undefined}
      className={cn(
        'p-4 rounded-xl',
        'bg-neutral-900 border border-neutral-800',
        'hover:border-neutral-700 transition-colors',
        onCardClick && 'cursor-pointer focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 focus:ring-offset-neutral-950'
      )}
    >
      {/* Header */}
      <div className="flex items-center gap-3 mb-4">
        <div className="relative">
          <Image
            src={user.avatar || 'https://api.dicebear.com/9.x/avataaars/png?seed=default'}
            alt={user.name}
            width={48}
            height={48}
      ...
      {/* Stats */}
      <div className={cn('grid gap-3 mb-4', monthRange ? 'grid-cols-3' : 'grid-cols-2')}>
        {monthRange && (
          <div className="p-3 rounded-lg bg-neutral-800/50">
            <div className="text-xs text-neutral-500 mb-1">Duration</div>
            <div className="text-sm font-medium text-white">{monthRange}</div>
          </div>
        )}
        <div className="p-3 rounded-lg bg-neutral-800/50">
      ...
      <div className="flex gap-2">
        <a
          ...
          aria-label={`Contact ${user.name} on Discord`}
          className={cn(
        ...
        <a
          ...
          rel="noopener noreferrer"
          onClick={handleLinkClick}
          aria-label={`View ${user.name}'s GitHub profile`}
          className={cn(
            'flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-lg transition-colors',
            user.githubUsername
