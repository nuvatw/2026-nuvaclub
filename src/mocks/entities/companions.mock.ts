...
// Helper functions
export const getCompanionsByType = (type: string): Companion[] =>
  MOCK_COMPANIONS.filter((c) => c.type === type);

export const getAvailableCompanions = (): Companion[] =>
  MOCK_COMPANIONS.filter((c) => c.isAvailable);

export const getCompanionById = (id: string): Companion | undefined =>
  MOCK_COMPANIONS.find((c) => c.id === id);

export const getNunuCompanions = (): Companion[] =>
  MOCK_COMPANIONS.filter((c) => c.type === 'nunu');

export const getCertifiedNunuCompanions = (): Companion[] =>
  MOCK_COMPANIONS.filter((c) => c.type === 'certified-nunu');

export const getShangzheCompanion = (): Companion | undefined =>
  MOCK_COMPANIONS.find((c) => c.type === 'shangzhe');
