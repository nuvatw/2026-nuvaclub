 ...
 *
 * Defines what each identity type can do in the application.
...
export const PERMISSION_MATRIX: Record<IdentityType, Permission[]> = {
  guest: ['learn:view_first_chapter', 'forum:read', 'sprint:view_projects', 'shop:browse'],
  explorer: EXPLORER_PERMISSIONS,
  'solo-traveler': SOLO_TRAVELER_PERMISSIONS,
};

// Helper functions
export const hasPermission = (identity: IdentityType, permission: Permission): boolean =>
  PERMISSION_MATRIX[identity]?.includes(permission) ?? false;

export const hasAllPermissions = (identity: IdentityType, permissions: Permission[]): boolean =>
  permissions.every((p) => hasPermission(identity, p));

export const hasAnyPermission = (identity: IdentityType, permissions: Permission[]): boolean =>
  permissions.some((p) => hasPermission(identity, p));

export const getPermissions = (identity: IdentityType): Permission[] =>
  PERMISSION_MATRIX[identity] ?? [];
