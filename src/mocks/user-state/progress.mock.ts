 ...
 *
 * Simulates watch progress for courses.
...
// Helper functions
export const getUserProgress = (userId: string, itemId: string): WatchProgress | undefined =>
  MOCK_USER_PROGRESS.find((p) => p.userId === userId && p.itemId === itemId);

export const getInProgressItems = (userId: string): WatchProgress[] =>
  MOCK_USER_PROGRESS.filter((p) => p.userId === userId && p.progressPercent > 0 && p.progressPercent < 100).sort(
    (a, b) => b.lastWatchedAt.getTime() - a.lastWatchedAt.getTime()
  );

export const getCompletedItems = (userId: string): WatchProgress[] =>
  MOCK_USER_PROGRESS.filter((p) => p.userId === userId && p.completedAt !== undefined);

export const hasAnyProgress = (userId: string): boolean =>
  MOCK_USER_PROGRESS.some((p) => p.userId === userId && p.progressPercent > 0);

export const getAllUserProgress = (userId: string): WatchProgress[] =>
  MOCK_USER_PROGRESS.filter((p) => p.userId === userId);

export const getProgressByItemId = (itemId: string): WatchProgress[] =>
  MOCK_USER_PROGRESS.filter((p) => p.itemId === itemId);
