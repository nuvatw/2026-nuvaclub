 ...
 *
 * Mock user favorites data using the 10 canonical users (user-1 through user-10).
 * Migrated from data/user-data/favorites.ts
 */
...
export function getUserFavorites(userId: string): UserFavorite[] {
  return MOCK_USER_FAVORITES.filter((f) => f.userId === userId);
}

export function getUserFavoritesByType(
  userId: string,
  itemType: UserFavorite['itemType']
): UserFavorite[] {
  return MOCK_USER_FAVORITES.filter((f) => f.userId === userId && f.itemType === itemType);
}

export function isItemFavorited(userId: string, itemId: string): boolean {
  return MOCK_USER_FAVORITES.some((f) => f.userId === userId && f.itemId === itemId);
}
