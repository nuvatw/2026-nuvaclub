...
export const EventsTable: EventProduct[] = [
  // ==========================================
...
/** @deprecated Use EventsTable instead */
export const MOCK_EVENTS = EventsTable;
...
export const getEventById = (id: string): EventProduct | undefined =>
  EventsTable.find((e) => e.id === id);

export const getUpcomingEvents = (): EventProduct[] =>
  EventsTable.filter((e) => e.date >= new Date()).sort(
    (a, b) => a.date.getTime() - b.date.getTime()
  );

export const getEventsByType = (eventType: EventProduct['eventType']): EventProduct[] =>
  EventsTable.filter((e) => e.eventType === eventType);

export const getFreeEvents = (): EventProduct[] =>
  EventsTable.filter((e) => e.price === 0);

export const getAvailableEvents = (): EventProduct[] =>
  EventsTable.filter((e) => e.remainingSeats > 0);

export const getPastEvents = (): EventProduct[] => {
  const now = new Date();
  return EventsTable.filter((e) => e.date <= now).sort((a, b) => b.date.getTime() - a.date.getTime());
};

export const getInPersonEvents = (): EventProduct[] =>
  EventsTable.filter((e) => e.eventType === 'in-person');

export const getOnlineEvents = (): EventProduct[] =>
  EventsTable.filter((e) => e.eventType === 'online');
...
export const getAllEvents = (): EventProduct[] => EventsTable;
